...
describe('SharePanel', () => {
  ...
  it('shows creating state and copies only after success', async () => {
    ...
    expect(await screen.findByText('仅限本机访问')).toBeInTheDocument()
    fireEvent.click(await screen.findByRole('button', { name: '创建本机访问链接' }))
    ...
    expect(useWorkbench.getState().toast).toMatchObject({ message: '本机访问链接已复制', variant: 'success' })
